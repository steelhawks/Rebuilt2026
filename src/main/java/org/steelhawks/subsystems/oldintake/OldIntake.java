...
public class OldIntake extends SubsystemBase {
	...
	private final OldIntakeIO io;

	private LoggedTunableNumber tuningVolts;
	private LoggedTunableNumber tuningAmps;

	private OldIntakeConstants.State desiredGoal = OldIntakeConstants.State.HOME;
	private TrapezoidProfile.State setpoint = new TrapezoidProfile.State();
	private TrapezoidProfile.State goal = new TrapezoidProfile.State();
	private boolean brakeModeEnabled = false;
	private boolean atGoal = false;

	public OldIntake(OldIntakeIO io) {
		this.io = io;
		profile =
			new TrapezoidProfile(
				new TrapezoidProfile.Constraints(
					OldIntakeConstants.MAX_VELOCITY_RAD_PER_SEC.get(),
					OldIntakeConstants.MAX_ACCEL_RAD_PER_SEC_SQ.get()));
	}
	...
	public void periodic() {
		...
		final boolean shouldRun =
			...
			&& (getPosition().getRadians() >= OldIntakeConstants.MIN_ROTATION.getRadians()
				&& getPosition().getRadians() <= OldIntakeConstants.MAX_ROTATION.getRadians());
		...
		if (Toggles.tuningMode.get()) {
			...
			LoggedTunableNumber.ifChanged(this.hashCode(), () -> {
				io.setPivotPID(
				OldIntakeConstants.kP.get(),
				OldIntakeConstants.kI.get(),
				OldIntakeConstants.kD.get());
			}, OldIntakeConstants.kP, OldIntakeConstants.kI, OldIntakeConstants.kD);
			if (OldIntakeConstants.MAX_VELOCITY_RAD_PER_SEC.hasChanged(hashCode())
			    || OldIntakeConstants.MAX_ACCEL_RAD_PER_SEC_SQ.hasChanged(hashCode())
            ) {
				profile =
					new TrapezoidProfile(
						new TrapezoidProfile.Constraints(
		...
		if (shouldRun) {
			...
			if (setpoint.position < OldIntakeConstants.MIN_ROTATION.getRadians()
			    || setpoint.position > OldIntakeConstants.MAX_ROTATION.getRadians()
            ) {
				setpoint =
					new TrapezoidProfile.State(
						MathUtil.clamp(
							setpoint.position,
							OldIntakeConstants.MIN_ROTATION.getRadians(),
							OldIntakeConstants.MAX_ROTATION.getRadians()),
						0.0);
			}
			atGoal = Math.abs(getPosition().getRadians() - goal.position) <= OldIntakeConstants.TOLERANCE;
			if (atGoal) {
				io.stopPivot();
			} else {
	...
	public void setDesiredState(OldIntakeConstants.State state) {
    ...
    public Command setDesiredStateCommand(OldIntakeConstants.State state) {
